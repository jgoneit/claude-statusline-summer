...
> [Claude Code](https://code.claude.com/)를 위한 여름 테마 상태줄 — 테마는 이모지가 아니라 **색**으로 표현됩니다.

[English](./README.md) | **한국어**

![claude-statusline-summer](./docs/claude-statusline-summer.png)

잔잔한 청록 → 타오르는 빨강으로 흐르는 **하나의 선셋 그라데이션 게이지**를 컨텍스트
사용량과 5h/7d rate limit에 똑같이 재사용합니다. 그래서 모든 게이지가 같은 언어로
말합니다: **찰수록 뜨거워 보인다.** 🔥

- 🎚️ **게이지 하나로 전부** — 컨텍스트 사용량 + 5h/7d rate limit이 같은 청록→빨강 그라데이션 공유
- 🌈 **트루컬러 + 256색 폴백** — 자동 감지, 띠 없음 (Apple Terminal.app에서도)
- 🌿 **git 한눈에** — 브랜치 · 스테이지 · 변경 · ahead/behind
- 💸 **세션 비용 & 시간** — 실시간 지출과 경과 시간
- 🎨 **테마 가능** — 팔레트는 `themes/<name>/colors.sh`에 (`summer` 기본 제공; `christmas`는 스캐폴드)
...
그다음 Claude Code 세션을 열면 — 새 상태줄이 보입니다. ✨

`install.sh`는 `jq`를 확인하고, `statusline.sh` + `themes/`를 `~/.claude/`로 복사한 뒤
`~/.claude/settings.json`에 등록합니다 — 기존 설정을 먼저 백업하고, 이미 다른 status
line이 있으면 교체 전에 물어봅니다. 다시 실행해도 안전합니다.

## 📋 요구사항

- 🐚 **bash** — macOS 기본 bash 3.2에서 동작
- 🔧 **[jq](https://jqlang.github.io/jq/)** — `brew install jq` / `sudo apt install jq`
- 🎨 **색을 지원하는 터미널** — 트루컬러(24-bit)를 자동 감지해 우선 사용하고, 없으면
  (예: Apple Terminal.app) 띠 없는 256색 팔레트로 폴백합니다.

<details>
<summary>🛠️ 수동 설치</summary>
...
- 🖥️ tmux에서 트루컬러를 쓰려면 `~/.tmux.conf`에 `set -ga terminal-overrides ",*:Tc"` 필요.
- ⏱️ `refreshInterval`(settings.json)은 스크립트를 주기적으로 재실행해서 유휴 상태에서도
